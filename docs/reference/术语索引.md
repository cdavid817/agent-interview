...
> 本文件保留在面试题「相关知识点」中至少出现 **5** 次的术语，并补充 Agent 领域核心术语；共 **650** 个去重术语，按 12 个主题分类。本页是分类索引，不把术语列表误称为完整定义。
>
> 当前覆盖 `docs/` 下 **88** 个题目文件、**1738** 处「相关知识点」。筛选规则由 `scripts/glossary_core_terms.txt` 与最小出现次数共同维护。
