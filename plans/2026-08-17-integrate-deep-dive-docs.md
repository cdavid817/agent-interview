...
- 单个题目文件不超过 50 题。
- **答案篇幅 250–550 字（去空白字符），复杂题可到 600。** 仓库现有产品章节的实测中位数：`tools-permissions.md` 463、`architecture-runtime.md` 278、`hooks-mcp.md` 272，最长 599。**不要为压字数牺牲考点**——宁可分点少而深，也不要把多个子系统各压成两句话。若某题的考点密度撑不下一题的篇幅，说明选题过宽，应在报告里指出而不是硬压。
- 一个子系统一题。若对照表里某题名义上覆盖三个独立子系统（如「认证、控制面与同步分享」），选其中考点最集中的一个深写，其余在相关知识点里点到即可，并在报告里说明取舍。
- 同一文件内稳定 ID 数字必须严格递增。
...
| OPC-012 | 12 | OpenCode 的跨工作区会话同步如何保证多设备一致性？写权冲突怎么裁决？ | 第二十章 |
| OPC-013 | 13 | OpenCode 从 V1 演进到 V2 最关键的架构改变是什么？迁移成本体现在哪里？ | 第二十六章 |

写作时逐题打开对应章节阅读后再落笔，不要凭标题臆测。行文结构：首段一句话给结论并加粗关键词，后续分点展开，最后一行写相关知识点。篇幅见 Global Constraints。
...
| OPC-022 | 9 | OpenCode 的工具系统如何表达权限？非沙箱边界体现在哪里？ | 第十七章 |
